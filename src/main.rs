...
// TODO fix set volume not affecting the playback volume on the sink
...
fn main() -> eframe::Result {
    ...
    let native_options = NativeOptions {
        viewport: viewport::ViewportBuilder::default()
            ...
            .with_icon(
                eframe::icon_data::from_png_bytes(consts::ASSET_ICON).expect("Failed loading icon"),
            ),
        ..Default::default()
    };
    ...
    eframe::run_native(
        "Friends Notifier",
        native_options,
        Box::new(|cc| Ok(Box::new(FriendsNotifierApp::new(cc)))),
    )
}
